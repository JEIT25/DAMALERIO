...
include __DIR__ . '/../includes/layout/sidebar.php'; ?>
    ...
    <script>
        ...
        function renderPagination(p) {
            ...
            info.textContent = `Showing ${p.total_requests > 0 ? start : 0}-${end} of ${p.total_requests} requests`;

            let html = '';
            html += `<button class="btn-secondary" style="padding: 0.25rem 0.5rem;" ${p.current_page === 1 ? 'disabled' : `onclick="loadRequests(${p.current_page - 1})"`}>&laquo;</button>`;

            for (let i = 1; i <= p.total_pages; i++) {
                if (i === 1 || i === p.total_pages || (i >= p.current_page - 1 && i <= p.current_page + 1)) {
                    html += `<button class="${i === p.current_page ? 'btn-primary' : 'btn-secondary'}" style="padding: 0.25rem 0.5rem; min-width: 2rem;" onclick="loadRequests(${i})">${i}</button>`;
                } else if (i === p.current_page - 2 || i === p.current_page + 2) {
                    html += `<span style="padding: 0 0.25rem;">...</span>`;
                }
            }

            html += `<button class="btn-secondary" style="padding: 0.25rem 0.5rem;" ${p.current_page === p.total_pages || p.total_pages === 0 ? 'disabled' : `onclick="loadRequests(${p.current_page + 1})"`}>&raquo;</button>`;
