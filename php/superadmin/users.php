...
<body>
    ...
    <script>
                    ...
                    if (data.users.length === 0) {
                        ...
                        data.users.forEach(u => {
                            ...
                            html += `<tr class="${rowClass}">
                                ...
                                <td><span class="status-badge no-dot ${roleClass}">${escapeHtml(u.role || 'consumer').toUpperCase()}</span></td>
        ...
        function blockUser(userId, action) {
            ...
            const targetUser = usersMap[userId];
            if (action === 'unblock' && targetUser && targetUser.role === 'superadmin') {
                if (!confirm("CRITICAL: Unblocking a Superadmin will automatically BLOCK your current account and log you out, as the system only allows one active Superadmin. Do you wish to proceed?")) {
                    return;
                }
            } else {
            ...
            fetch(api + '/superadmin_user_block.php', {
                ...
                if (d.success) { 
                    if (d.superadmin_swap) {
                        alert("SECURITY ACTION: You have successfully unblocked another Superadmin. You will now be logged out. Please log in with the other Superadmin's credentials.");
        ...
        const privilegesConfig = {
            ...
            'consumer': [
                ...
                { desc: 'Manage Store & Menus',                        has: false },
            ],
        };
        ...
        function viewPrivileges(role, name) {
            ...
            const listHtml = privs.map(p => `
                <div style="display:flex; align-items:center; gap:12px; padding:10px 12px; border-radius:8px;
                    ...
                    <span style="font-size:0.875rem; font-weight:500; color:${p.has ? '#166534' : '#64748b'}">${p.desc}</span>
                </div>`
            ).join('');
        ...
        AdminUserValidation.init(document.getElementById('userForm'), { apiBase: api });
        loadUsers();
    </script>
</body>
</html>
