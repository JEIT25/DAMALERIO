...
if ($result && $result->num_rows === 0) {
    $conn->query("ALTER TABLE users ADD COLUMN status ENUM('pending','active','rejected') NOT NULL DEFAULT 'pending' AFTER role");
    $conn->query("UPDATE users SET status = 'active'");
} else {
    $conn->query("ALTER TABLE users MODIFY COLUMN status ENUM('pending','active','rejected') NOT NULL DEFAULT 'pending'");
    $conn->query("UPDATE users SET status = 'active' WHERE status = 'approved'");
}
$conn->query("UPDATE users SET status = 'active' WHERE status = 'pending' AND id NOT IN (SELECT target_id FROM user_block_requests WHERE request_type = 'registration' AND status = 'pending')");
