-- ============================================================
-- FoodGrab: Enhanced Security Features (FIXED VERSION)
...
-- Add 2 additional security questions to users table (FIXED VERSION)
DROP PROCEDURE IF EXISTS add_security_questions;
DELIMITER //
CREATE PROCEDURE add_security_questions()
BEGIN
  -- First check if secure_answer exists, if not add it with secure_question
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'secure_answer'
  ) THEN
    -- Add both secure_question and secure_answer if they don't exist
    ALTER TABLE `users`
      ADD COLUMN `secure_question` varchar(100) DEFAULT NULL AFTER `role`,
      ADD COLUMN `secure_answer` varchar(255) DEFAULT NULL AFTER `secure_question`;
  END IF;
  
  -- Now add the additional security questions if they don't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.COLUMNS
